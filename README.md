...
- **部分ラスタライズ**: 特定のページだけを画像化し、他のページはベクターのまま維持可能
- **暗号化ファイル対応**: 暗号化されたPDFも全ページ再構築モードで変換可能
...
1. PDFファイルを選択
2. 必要に応じてDPI（解像度）を調整
3. 必要に応じてページ範囲（例: `1, 3-5`）を指定（空欄で全ページ）
4. 「変換」ボタンをクリック
5. 変換されたPDFをダウンロード

暗号化されたPDFの場合、自動的に全ページラスタライズモードに切り替わるか、確認ダイアログが表示されます。
...
# 特定のページのみ画像化（1と3-5ページ）
pdf_rasterizer --pages "1, 3-5" input.pdf output.pdf
```
